...
ob_start();
$sessionToken = getACookieNR($username, $password);
ob_end_clean();
...
if($sessionToken === "")
...
else
	{
		//echo $sessionToken;
		//echo "<br>";
		getNextRace($appKey, $sessionToken,$eventID);
...
function getACookieNR($username, $password){
...
function getNextRace($appKey, $sessionToken, $eventTypeID)
{
...
$params = '{"filter": 
    ...
    $jsonResponse = nextRaceApingRequest($appKey, $sessionToken, 'listMarketCatalogue', $params);
...
function nextRaceApingRequest($appKey, $sessionToken, $operation, $params)
{
    $ch = curl_init();
